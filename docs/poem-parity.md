...
| ~~MCP Server(poem-mcpserver相当)~~ | `crates/open-runo-router/src/mcp.rs`(`POST /mcp`、新規依存無し) | ✅ 完了(2026-07-12)。Streamable HTTP transportの単純系(1リクエスト→1レスポンス、SSE無し)でJSON-RPC 2.0を実装。`initialize`/`tools/list`/`tools/call`に対応、実ツール2種(`health_check`・`self_issue_api_key`——いずれも既存の`GET /health`・`POST /api/keys/self-issue`と同じ本番ロジックを共有、MCP専用のスタブではない)を公開。実バイナリ+curlでinitialize→tools/list→tools/call(self_issue_api_key、実際に有効なAPIキーが返る)の一連を確認済み。Resources/Promptsは未対応(Toolsのみ) |
